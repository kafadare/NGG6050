...
% EXERCISE:
function likelihood = likelihood_for_each_data_point(model_params, RT)
    % Compute the likelihood for each data point given the model parameters
    % (You need to define this function based on your specific model)

    % Example: Assuming a normal distribution
    mu = model_params(1);
    sigma = model_params(2);

    likelihood = normpdf(RT, mu, sigma); % Adjust this based on your specific model
end
laterErrFcn = @(model_params, fits) -sum(log(likelihood_for_each_data_point(model_params, fits)));
...
% plot RT values cumulative percent probability and the log likelihood model
% on the same plot to compare the fit of the model to the later_getData

% plot the model_params
y_pred = laterErrFcn(RTs, fits);
% Compute the empirical cumulative distribution function (ECDF)
[cdf, x] = ecdf(RTs);
% Plot the cumulative distribution function in blue
figure;
plot(x, cdf, 'LineWidth', 2, 'Color', 'blue');
hold on;
scatter(RTs, y_pred, 'o', 'DisplayName', 'Data Points');
hold on;
xlabel('RTs');
ylabel('Cumulative Percent Probability');
title('Data Points and Fitted Log-Likelihood Model');
legend('show');
grid on;
hold off;
